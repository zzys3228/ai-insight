...
Fetches robot company news and announcements.
"""
...
class RobotFetcher:
    """Fetcher for robot company news"""
    ...
    def _parse_article(self, html: str, url: str, company: str) -> dict:
        ...
        # Title
        title = ''
        title_tag = soup.find('h1') or soup.find(class_=['post-title', 'article-title', 'entry-title'])
        if title_tag:
    ...
    def discover_news(self, news_url: str) -> list:
        """Discover news links from news listing page"""
        ...
        for link in soup.find_all('a', href=True):
            href = link['href']
            if any(skip in href for skip in ['/category/', '/tag/', '#', 'login', 'subscribe']):
                continue
            if '/news/' in href or '/blog/' in href or '/post/' in href or '/press/' in href:
    ...
    def format_as_markdown(self, article: dict) -> str:
        ...
        md = format_markdown_frontmatter(
            ...
            url=article['url'],
            date=article['date'],
            category=f"robot/{article['company']}",
            translated=True,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Fetch robot news')
    parser.add_argument('--url', type=str, help='Article URL')
    parser.add_argument('--company', type=str, required=True, help='Company name')
    parser.add_argument('--discover', action='store_true', help='Discover news from listing page')

    args = parser.parse_args()
    fetcher = RobotFetcher()

    if args.discover and args.url:
        print(f"Discovering news from {args.url}...")
        links = fetcher.discover_news(args.url)
        ...
        try:
            ...
            output_dir = Path(STORAGE_BASE) / 'robot' / args.company
